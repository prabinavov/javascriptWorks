...
1//is exist any contries less tless than population 11500000000

var isExist=countries.some(c=>c.population<115000000)
console.log(isExist);


//2. is there any contries with more than 2 languge

var isExist=countries.some(c=>c.language.length>2)

console.log(isExist);

//3 is there any contiers with more than 3 borders

var isInclude=countries.some(c=>c.borders.length>7)

console.log(isInclude);

//4. take the details of india

var India=countries.find(d=>d.name=="India")
...
var Alpha=countries.find(q=>q.alpha2cod=="IQ")

console.log(Alpha);


//6. find the details of currency in CNY

var courrency=countries.find(c=>c.currency.toLowerCase()=="cny")

console.log(courrency);
//7 . fore each contries names

countries.filter(c=> c.borders.length>2).forEach(c=>console.log(c.name));
//8. list the  name of countries with cureency as eur
console.log("eur using countries");
...
var totelpopulation=countries.reduce((p1,p2)=>p1+p2.population,0)



//console.log(totelpopulation);
...
for(let c of countries){
   ...
   let curre=c.currency
    

    if( curre in sameContries){
        sameContries[curre]+=1
...
console.log(sameContries);

console.log(Object.entries(sameContries).sort((c1,c2)=>c1[1]-c2[1]));



//3. Sort all countries in alphabetical order (by country name).
//  sorrt alkl countries in alphabetic order ( by county name )


//4. Count how many countries use each language.
// count hou=w many countruy use eachlanguge
